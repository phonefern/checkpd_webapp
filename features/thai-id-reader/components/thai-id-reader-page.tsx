...
export function ThaiIdReaderPage() {
  ...
  return <SidebarLayout activePath="/pages/thai-id-reader" mainClassName="bg-gray-50">
    <div className="mx-auto w-full max-w-6xl space-y-6 p-4 md:p-8">
      ...
      <Card className="rounded-2xl shadow-sm"><CardHeader><CardTitle>Thai ID Card Reader</CardTitle><CardDescription>Insert a card, then read it from this clinic computer.</CardDescription></CardHeader><CardContent className="space-y-4">{error ? <div className="rounded-xl border border-destructive/30 bg-destructive/10 p-4 text-sm text-destructive"><div className="flex items-start gap-2"><CircleAlert className="mt-0.5 h-4 w-4 shrink-0" /><span>{error}</span></div><p className="mt-2 text-destructive/80">Check that SolId Reader is running, the reader is connected, and a card is inserted.</p></div> : null}<div className="flex flex-wrap gap-3"><Button size="lg" className="min-h-14 w-full sm:w-auto" onClick={() => void handleRead()} disabled={!canRead}>{isReading ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : <CreditCard className="mr-2 h-5 w-5" />}{isReading ? "Reading card…" : "Read Thai ID Card"}</Button>{bridgeStatus !== "connected" && bridgeStatus !== "checking" && bridgeStatus !== "not_configured" ? <Button size="lg" variant="outline" className="min-h-14 w-full sm:w-auto" onClick={() => beginRecovery(false)}>Open or install SolId Reader</Button> : null}</div>{!canRead && !isReading ? <p className="text-sm text-muted-foreground">{bridgeStatus === "not_configured" ? "Ask an administrator to configure the SolId Reader bridge token for this web app." : "Connect SolId Reader before reading a card."}</p> : null}</CardContent></Card>
