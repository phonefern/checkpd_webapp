-- PLAN-027: Aggregate dashboard stats in Postgres.
...
WITH params AS (
  SELECT
    ...
    CASE
      ...
      ELSE 'all'
    END AS risk_filter,
    (p_start IS NOT NULL OR p_end IS NOT NULL) AS has_date_range
),
...
risk_bucket_by_user AS MATERIALIZED (
  SELECT
    b.id,
    CASE
...
download_raw AS (
  ...
  FROM checkpd.users u
  CROSS JOIN params p
  WHERE NOT p.has_date_range
     OR (
       (p.start_date IS NULL OR u.firebase_created_at >= p.start_date)
       AND (p.end_date IS NULL OR u.firebase_created_at < (p.end_date + 1))
       AND (p.province IS NULL OR u.province = p.province)
       AND (p.area IS NULL OR u.area = p.area)
     )
),
risk_target_count AS (
  SELECT COUNT(*)::int AS count
  FROM risk_bucket_by_user rb
  CROSS JOIN params p
  WHERE (p.risk_filter = 'risk' AND rb.bucket = 'risk')
     OR (p.risk_filter = 'no_risk' AND rb.bucket = 'normal')
     OR (p.risk_filter = 'unknown' AND rb.bucket = 'pending')
),
download_count AS (
  SELECT CASE
    WHEN p.has_date_range AND p.risk_filter <> 'all'
      THEN LEAST(download_raw.count, risk_target_count.count)
    ELSE download_raw.count
  END::int AS count
  FROM params p
  CROSS JOIN download_raw
  CROSS JOIN risk_target_count
)
