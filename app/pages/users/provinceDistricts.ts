// Thailand province -> อำเภอ (district) reference list.
// Source: app/pages/users/district.md (BigQuery REGEXP_EXTRACT case list —
// parsed once into a static map so the district filter can cascade off the
// selected province instantly, without a database round trip). Values match
// the exact district names checkpd/public.users.area is populated with.
export const PROVINCE_DISTRICTS: Record<string, string[]> = {
  ...
  "นครปฐม": ["เมืองนครปฐม", "นครชัยศรี", "สามพราน", "ดอนตูม", "บางเลน", "กำแพงแสน", "พุทธมณฑล"],
  ...
  "ประจวบคีรีขันธ์": ["เมืองประจวบคีรีขันธ์", "บางสะพาน", "บางสะพานน้อย", "ปราณบุรี", "หัวหิน", "กุยบุรี", "ทับสะแก", "สามร้อยยอด"],
  ...
  "พัทลุง": ["เมืองพัทลุง", "กงหรา", "เขาชัยสน", "ตะโหมด", "ควนขนุน", "ปากพะยูน", "ศรีบรรพต", "ศรีนครินทร์", "ป่าบอน", "บางแก้ว", "ป่าพะยอม"],
  ...
  "สกลนคร": ["เมืองสกลนคร", "กุสุมาลย์", "กุดบาก", "พรรณานิคม", "พังโคน", "วาริชภูมิ", "วานรนิวาส", "คำตากล้า", "บ้านม่วง", "อากาศอำนวย", "สว่างแดนดิน", "ส่องดาว", "เต่างอย", "โคกศรีสุพรรณ", "เจริญศิลป์", "โพนนาแก้ว", "ภูพาน", "ฮางโฮง", "นิคมน้ำอูน"],
...
// Flattened, de-duplicated, alphabetized — used when no province filter is
// selected yet (mirrors the previous nationwide area dropdown).
export const ALL_DISTRICTS: string[] = Array.from(
  new Set(Object.values(PROVINCE_DISTRICTS).flat())
).sort((a, b) => a.localeCompare(b, 'th'))

// Direct port of district.md's REGEXP_EXTRACT case list: within the given
// province's district names, return the first one that appears as a
// substring of liveaddress (same "first alternation match" semantics the
// BigQuery regex has — this is a free-text heuristic, not a guarantee).
export function extractDistrict(liveaddress?: string | null, province?: string | null): string | null {
  if (!liveaddress || !province) return null
  const districts = PROVINCE_DISTRICTS[province]
  if (!districts) return null
  for (const district of districts) {
    if (liveaddress.includes(district)) return district
  }
  return null
}
