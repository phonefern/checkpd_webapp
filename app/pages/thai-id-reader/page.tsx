...
import { BadgeCheck, CircleAlert, CreditCard, Loader2, RotateCcw, Trash2 } from "lucide-react";
...
type CardRead = { id: string; readAt: Date; data: ThaiIdCardPayload };
...
export default function ThaiIdReaderPage() {
  ...
  const [reads, setReads] = useState<CardRead[]>([]);
  ...
  const checkBridge = useCallback(async () => {
    ...
    try {
      ...
      setBridgeStatus(checkError instanceof LocalBridgeConfigurationError ? "not_configured" : "not_running");
    }
  }, []);
  ...
  return <SidebarLayout activePath="/pages/thai-id-reader" mainClassName="bg-gray-50">
    <div className="mx-auto w-full max-w-6xl space-y-6 p-4 md:p-8">
      ...
      <Card className="rounded-2xl shadow-sm"><CardHeader><CardTitle>Thai ID Card Reader</CardTitle><CardDescription>Insert a card, then read it from this clinic computer.</CardDescription></CardHeader><CardContent className="space-y-4">{error ? <div className="rounded-xl border border-destructive/30 bg-destructive/10 p-4 text-sm text-destructive"><div className="flex items-start gap-2"><CircleAlert className="mt-0.5 h-4 w-4 shrink-0" /><span>{error}</span></div><p className="mt-2 text-destructive/80">Check that SolId Reader is running, the reader is connected, and a card is inserted.</p></div> : null}<Button size="lg" className="min-h-14 w-full sm:w-auto" onClick={() => void handleRead()} disabled={!canRead}>{isReading ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : <CreditCard className="mr-2 h-5 w-5" />}{isReading ? "Reading card…" : "Read Thai ID Card"}</Button>{!canRead && !isReading ? <p className="text-sm text-muted-foreground">Connect SolId Reader and configure its bridge token before reading a card.</p> : null}</CardContent></Card>
      {latestRead ? <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}><Card className="rounded-2xl border-emerald-200 shadow-sm"><CardHeader className="flex-row items-center gap-2 space-y-0"><BadgeCheck className="h-5 w-5 text-emerald-600" /><CardTitle className="text-base">Latest card read</CardTitle></CardHeader><CardContent className="flex flex-wrap items-start gap-5">{latestRead.data.photoAsBase64Uri ? <img src={latestRead.data.photoAsBase64Uri} alt="Thai ID card holder" className="h-28 w-24 rounded-lg border object-cover" /> : null}<div className="space-y-1 text-sm"><p className="font-semibold">{latestRead.data.fullNameTH || latestRead.data.fullNameEN}</p><p>{latestRead.data.citizenID}</p><p className="text-muted-foreground">{latestRead.data.dateOfBirth} · {genderLabel(latestRead.data.gender)}</p></div></CardContent></Card></motion.div> : null}
      <Card className="rounded-2xl shadow-sm"><CardHeader className="flex-row flex-wrap items-center justify-between gap-3 space-y-0"><div><CardTitle>Temporary Card Read Results</CardTitle><CardDescription>Visible only while this page remains open. Nothing is saved.</CardDescription></div><Button variant="outline" onClick={() => { setReads([]); setExpandedReadId(null); }} disabled={reads.length === 0}><Trash2 className="mr-2 h-4 w-4" />Clear results</Button></CardHeader><CardContent>{reads.length === 0 ? <div className="rounded-xl border border-dashed py-12 text-center text-sm text-muted-foreground">No cards read in this session.</div> : <div className="overflow-x-auto rounded-xl border"><Table><TableHeader><TableRow><TableHead>Photo</TableHead><TableHead>Thai ID</TableHead><TableHead>Name</TableHead><TableHead>Birth date</TableHead><TableHead>Gender</TableHead><TableHead>Read at</TableHead><TableHead>Details</TableHead></TableRow></TableHeader><TableBody>{reads.map((read) => <Fragment key={read.id}><TableRow><TableCell>{read.data.photoAsBase64Uri ? <img src={read.data.photoAsBase64Uri} alt="Thai ID card holder" className="h-10 w-8 rounded object-cover" /> : "-"}</TableCell><TableCell className="font-mono text-xs">{read.data.citizenID}</TableCell><TableCell>{read.data.fullNameTH || read.data.fullNameEN || "-"}</TableCell><TableCell>{read.data.dateOfBirth || "-"}</TableCell><TableCell>{genderLabel(read.data.gender)}</TableCell><TableCell className="whitespace-nowrap text-xs">{formatDate(read.readAt)}</TableCell><TableCell><Button size="sm" variant="outline" onClick={() => setExpandedReadId((current) => current === read.id ? null : read.id)}>{expandedReadId === read.id ? "Hide" : "View"}</Button></TableCell></TableRow>{expandedReadId === read.id ? <TableRow><TableCell colSpan={7} className="bg-muted/30"><div className="grid gap-4 p-2 text-sm md:grid-cols-2"><p><span className="text-muted-foreground">Thai name:</span> {read.data.fullNameTH || "-"}</p><p><span className="text-muted-foreground">English name:</span> {read.data.fullNameEN || "-"}</p><p><span className="text-muted-foreground">Card issuer:</span> {read.data.cardIssuer || "-"}</p><p><span className="text-muted-foreground">Issue / expiry:</span> {read.data.issueDate || "-"} / {read.data.expireDate || "-"}</p><p className="md:col-span-2"><span className="text-muted-foreground">Address:</span> {read.data.address || "-"}</p></div></TableCell></TableRow> : null}</Fragment>)}</TableBody></Table></div>}</CardContent></Card>
    </div>
  </SidebarLayout>;
}
